...
import com.WdkDrakor.WdkDrakor.Companion.cinemaOSApi
import com.WdkDrakor.WdkDrakor.Companion.Player4uApi
import com.WdkDrakor.WdkDrakor.Companion.idlixAPI
import com.WdkDrakor.WdkDrakor.Companion.RiveStreamAPI
// Import tambahan untuk Wdkmoviebox2
import javax.crypto.Mac
import javax.crypto.spec.SecretKeySpec
import java.security.MessageDigest
import android.net.Uri

object WdkDrakorExtractor : WdkDrakor() {
    ...
    // ================== WdkDEWASA / DRAMAFULL SOURCE ==================
    @Suppress("UNCHECKED_CAST")
    suspend fun invokeWdkDewasa(
        title: String,
        year: Int?,
        season: Int?,
        episode: Int?,
        ...
        val cleanQuery = WdkDewasaHelper.normalizeQuery(title)
        val encodedQuery = URLEncoder.encode(cleanQuery, "UTF-8").replace("+", "%20")
        val searchUrl = "$baseUrl/api/live-search/$encodedQuery"

        try {
            val searchRes = app.get(searchUrl, headers = WdkDewasaHelper.headers).parsedSafe<WdkDewasaSearchResponse>()
            
            val matchedItem = searchRes?.data?.find { item ->
                val itemTitle = item.title ?: item.name ?: ""
                WdkDewasaHelper.isFuzzyMatch(title, itemTitle)
            ...
            val doc = app.get(targetUrl, headers = WdkDewasaHelper.headers).document
            ...
            val docPage = app.get(targetUrl, headers = WdkDewasaHelper.headers).document
            ...
            val jsonResponseText = app.get(signedUrl, referer = targetUrl, headers = WdkDewasaHelper.headers).text
    ...
    // ================== WdkMOVIEBOX SOURCE (FIXED) ==================
    suspend fun invokeWdkmoviebox(
        title: String,
        year: Int?,
        season: Int?,
        episode: Int?,
        ...
        val items = tryParseJson<WdkmovieboxResponse>(searchRes)?.data?.items ?: return
        ...
        val streams = tryParseJson<WdkmovieboxResponse>(playRes)?.data?.streams ?: return
        ...
        if (id != null) {
            ...
            app.get(subUrl, referer = validReferer).parsedSafe<WdkmovieboxResponse>()?.data?.captions?.forEach { sub ->
                subtitleCallback.invoke(
                    newSubtitleFile(
                        sub.lanName ?: "Unknown",
    ...
    suspend fun invokeVidsrccc(
        ...
        val url = if (season == null) {
            "${WdkDrakor.vidsrcccAPI}/v2/embed/movie/$tmdbId"
        } else {
            "${WdkDrakor.vidsrcccAPI}/v2/embed/tv/$tmdbId/$season/$episode"
        }
        ...
        val serverUrl = if (season == null) {
            "${WdkDrakor.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=movie&v=$v&vrf=$vrf&imdbId=$imdbId"
        } else {
            "${WdkDrakor.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=tv&v=$v&vrf=$vrf&imdbId=$imdbId&season=$season&episode=$episode"
        }

        app.get(serverUrl).parsedSafe<VidsrcccResponse>()?.data?.amap {
            val sources =
                app.get("${WdkDrakor.vidsrcccAPI}/api/source/${it.hash}").parsedSafe<VidsrcccResult>()?.data
                    ?: return@amap
                ...
                it.name.equals("VidPlay") -> {
                    ...
                    callback.invoke(
                        newExtractorLink(
                            ...
                            this.referer = "${WdkDrakor.vidsrcccAPI}/"
                        }
                    )
    ...
    suspend fun invokeVidsrc(
        ...
        val url = if (season == null) {
            "${WdkDrakor.vidSrcAPI}/embed/movie?imdb=$imdbId"
        } else {
            "${WdkDrakor.vidSrcAPI}/embed/tv?imdb=$imdbId&season=$season&episode=$episode"
        }
    ...
    suspend fun invokeXprime(
        ...
        runAllAsync(
            {
                val url = if (season == null) {
                    "${WdkDrakor.xprimeAPI}/${servers.first()}?id=$tmdbId"
                } else {
                    "${WdkDrakor.xprimeAPI}/${servers.first()}?id=$tmdbId&season=$season&episode=$episode"
                }
            ...
            {
                val url = if (season == null) {
                    "${WdkDrakor.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year"
                } else {
                    "${WdkDrakor.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year&season=$season&episode=$episode"
                }
    ...
    suspend fun invokeWatchsomuch(
        ...
        val epsId = app.post(
            "${WdkDrakor.watchSomuchAPI}/Watch/ajMovieTorrents.aspx", data = mapOf(
                "index" to "0",
                "mid" to "$id",
        ...
        val subUrl = if (season == null) {
            "${WdkDrakor.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part="
        } else {
            "${WdkDrakor.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part=S${seasonSlug}E${episodeSlug}"
        }
        ...
        app.get(subUrl).parsedSafe<WatchsomuchSubResponses>()?.subtitles?.map { sub ->
            subtitleCallback.invoke(
                newSubtitleFile(
                    ...
                    fixUrl(sub.url ?: return@map null, WdkDrakor.watchSomuchAPI)
                )
            )
        }
    }
    ...
    suspend fun invokeMapple(
        ...
        val url = if (season == null) {
            "${WdkDrakor.mappleAPI}/watch/$mediaType/$tmdbId"
        } else {
            "${WdkDrakor.mappleAPI}/watch/$mediaType/$season-$episode/$tmdbId"
        }
        ...
        callback.invoke(
            newExtractorLink(
                ...
                this.referer = "${WdkDrakor.mappleAPI}/"
                this.headers = mapOf(
                    "Accept" to "*/*"
                )
            }
        ...
        val subRes = app.get(
            "${WdkDrakor.mappleAPI}/api/subtitles?id=$tmdbId&mediaType=$mediaType${if (season == null) "" else "&season=1&episode=1"}",
            referer = "${WdkDrakor.mappleAPI}/"
        ).text
        tryParseJson<ArrayList<MappleSubtitle>>(subRes)?.map { subtitle ->
            subtitleCallback.invoke(
                newSubtitleFile(
                    subtitle.display ?: "",
                    fixUrl(subtitle.url ?: return@map, WdkDrakor.mappleAPI)
                )
            )
        }
    }
    ...
    suspend fun invokeVidlink(
        ...
        val url = if (season == null) {
            "${WdkDrakor.vidlinkAPI}/$type/$tmdbId"
        } else {
            "${WdkDrakor.vidlinkAPI}/$type/$tmdbId/$season/$episode"
        }

        val videoLink = app.get(
            url, interceptor = WebViewResolver(
                Regex("""${WdkDrakor.vidlinkAPI}/api/b/$type/A{32}"""), timeout = 15_000L
            )
        ...
        callback.invoke(
            newExtractorLink(
                ...
                this.referer = "${WdkDrakor.vidlinkAPI}/"
            }
        )
    }
    ...
    suspend fun invokeVidfast(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) {
            "${WdkDrakor.vidfastAPI}/$type/$tmdbId"
        } else {
            "${WdkDrakor.vidfastAPI}/$type/$tmdbId/$season/$episode"
        }

        val res = app.get(
            url, interceptor = WebViewResolver(
                Regex("""${WdkDrakor.vidfastAPI}/$module/JEwECseLZdY"""),
                timeout = 15_000L
            )
        ).text
        ...
        tryParseJson<ArrayList<VidFastServers>>(res)?.filter { it.description?.contains("Original audio") == true }
            ?.amapIndexed { index, server ->
                val source =
                    app.get("${WdkDrakor.vidfastAPI}/$module/Sdoi/${server.data}", referer = "${WdkDrakor.vidfastAPI}/")
    ...
    suspend fun invokeWyzie(
        ...
        val url = if (season == null) {
            "${WdkDrakor.wyzieAPI}/search?id=$tmdbId"
        } else {
            "${WdkDrakor.wyzieAPI}/search?id=$tmdbId&season=$season&episode=$episode"
        }
    ...
    suspend fun invokeVixsrc(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) {
            "${WdkDrakor.vixsrcAPI}/$type/$tmdbId"
        } else {
            "${WdkDrakor.vixsrcAPI}/$type/$tmdbId/$season/$episode"
        }
        ...
        listOf(
            ...
            VixsrcSource("Vixsrc [Beta]", video2, "${WdkDrakor.mappleAPI}/"),
        ).map {
            callback.invoke(
                newExtractorLink(
                    it.name,
    ...
    suspend fun invokeSuperembed(
        ...
        val token = app.get("${WdkDrakor.superembedAPI}/directstream.php?video_id=$tmdbId&tmdb=1$path").url.substringAfter(
            "?play="
        )
    ...
    suspend fun invokeVidrock(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = "${WdkDrakor.vidrockAPI}/$type/$tmdbId${if (type == "movie") "" else "/$season/$episode"}"
        val encryptData = VidrockHelper.encrypt(tmdbId, type, season, episode)

        app.get("${WdkDrakor.vidrockAPI}/api/$type/$encryptData", referer = url).parsedSafe<LinkedHashMap<String, HashMap<String, String>>>()
            ?.map { source ->
                if (source.key == "source2") {
                    val json = app.get(source.value["url"] ?: return@map, referer = "${WdkDrakor.vidrockAPI}/").text
                    tryParseJson<ArrayList<VidrockSource>>(json)?.reversed()?.map mirror@{ it ->
                        callback.invoke(
                            newExtractorLink(
                                ...
                                this.headers = mapOf(
                                    "Range" to "bytes=0-",
                                    "Referer" to "${WdkDrakor.vidrockAPI}/"
                                )
                            }
                        )
                    }
                    ...
                    callback.invoke(
                        newExtractorLink(
                            ...
                            this.referer = "${WdkDrakor.vidrockAPI}/"
                            this.headers = mapOf(
                                "Origin" to WdkDrakor.vidrockAPI
                            )
                        }
                    )
                }
    ...
    // ================== WdkMOVIEBOX 2 SOURCE (NEW) ==================
    suspend fun invokeWdkmoviebox2(
        title: String,
        year: Int?,
        season: Int?,
        episode: Int?,
        ...
        val headersPlay = Wdkmoviebox2Helper.getHeaders(playUrl, null, "GET")

        val playRes = app.get(playUrl, headers = headersPlay).parsedSafe<Wdkmoviebox2PlayResponse>()
        ...
        streams.forEach { stream ->
            ...
            if (stream.id != null) {
                ...
                val headersSub = Wdkmoviebox2Helper.getHeaders(subUrl, null, "GET")
                
                app.get(subUrl, headers = headersSub).parsedSafe<Wdkmoviebox2SubtitleResponse>()?.data?.extCaptions?.forEach { cap ->
    ...
    // Helper Object untuk Enkripsi Wdkmoviebox2 (Private di dalam Extractor)
    private object Wdkmoviebox2Helper {
